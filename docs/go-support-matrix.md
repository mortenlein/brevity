...
| Runtime execution records and manual launch | Native list, inspect, plan-from-reservation, readiness transitions, preflight, launch-dry-run, and single foreground provider launch for `.brevity\runtime-executions.json` with observational run-history append; native runtime state and Bubble Tea expose compact execution visibility | Not PowerShell-owned | Go | execution metadata / manual provider-executing mutation | unit and fake-provider tests | Bubble Tea read-only visibility | migrated execution contract | Keep scheduler loops, queue draining, and task workflow mutation out of execution launch |
