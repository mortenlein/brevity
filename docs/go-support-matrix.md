...
| Runtime execution records | Native list, inspect, and plan-from-reservation for `.brevity\runtime-executions.json` | Not PowerShell-owned | Go | inert execution intent metadata / read-only diagnostics | unit tests | none | migrated contract | Keep provider execution out of execution-record v1 |
