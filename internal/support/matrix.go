...
var Matrix = []Capability{
	...
	{"runtime-execution-records", "Runtime execution record persistence, inspection, and manual launch", "Native list, inspect, plan-from-reservation, readiness transitions, preflight, launch-dry-run, and single foreground provider launch for runtime-executions.json with observational run-history append", "Not PowerShell-owned", "Go", "execution metadata / manual provider-executing mutation", "unit and fake-provider tests", "Bubble Tea read-only visibility", "execution contract migrated", "Keep scheduler loops, queue draining, and task workflow mutation out of execution launch", "none"},
